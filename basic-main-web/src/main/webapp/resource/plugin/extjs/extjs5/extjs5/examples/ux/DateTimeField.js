...
Ext.define('app.ux.DateTimeField', {
    ...
    onExpand: function() {
		...
		var temp_format = me.format;
		if( temp_format.indexOf('H')>0 ){
			me.picker.hour.setValue(value.getHours());
		}else{
			me.picker.hour.setValue(0);
			me.picker.hour.setDisabled(true);
		}
		
		if( temp_format.indexOf('i')>0 ){
			me.picker.minute.setValue(value.getMinutes());
		}else{
			me.picker.minute.setValue(0);
			me.picker.minute.setDisabled(true);
		}
		
		if( temp_format.indexOf('s')>0 ){
			me.picker.second.setValue(value.getSeconds());
		}else{
			me.picker.second.setValue(0);
			me.picker.second.setDisabled(true);
		}
