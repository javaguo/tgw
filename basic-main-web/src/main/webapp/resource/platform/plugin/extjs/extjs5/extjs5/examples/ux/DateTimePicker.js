   ...
   // monthNames:['f','r'],
    ...
    renderTpl: [
        '<div id="{id}-innerEl" data-ref="innerEl" role="presentation">',
            ...
            '<tpl if="showToday">',
			  ...
			  //'<div  id="{id}-footerEl" role="presentation" class="{baseCls}-footer">{%this.renderHour(values, out)%}&nbsp;&nbsp;{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}<center>{%this.renderTodayBtn(values, out)%}&nbsp;&nbsp;{%this.renderClearBtn(values, out)%}</center></div>',
            '</tpl>',
    ...
    selectToday: function() {
        ...
        if (btn && !btn.disabled) {
			...
			me.hour.setValue( temp_date.getHours() );
			me.minute.setValue( temp_date.getMinutes() );
			me.second.setValue( temp_date.getSeconds() );
    ...
    selectedUpdate: function(date) {
		//此处只比较年月日，将时分秒设置为0，比较完后还原
		var temp_h = date.getHours();
		var temp_m = date.getMinutes();
		var temp_s = date.getSeconds();
		date.setHours(0);
        date.setMinutes(0);
        date.setSeconds(0);
        ...
        //还原时间
		date.setHours(temp_h);
        date.setMinutes(temp_m);
        date.setSeconds(temp_s);
